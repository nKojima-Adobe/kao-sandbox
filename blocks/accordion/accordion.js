 ...
 * @property {string} [accordionHeader] Optional header text displayed above the list
...
/**
 * Decorates a accordion block with interactive behavior and accessibility.
...
export default function decorate(block) {
  ...
  if (accordionHeader || derivedHeader) {
    const headerEl = document.createElement('h3');
    headerEl.className = 'accordion-block-header';
    const rawContent = accordionHeader || derivedHeader;
  ...
  block.classList.add(`accordion-${normalized}`);
  ...
  const headerSibling = block.querySelector('.accordion-block-header');
  ...
  remainingRows.forEach((row) => {
    ...
    headerClone.className = 'accordion-item-title';
    ...
    [...body.children].forEach((child) => {
      if (!mediaTagged && child.querySelector('picture, img')) {
        child.classList.add('accordion-item-media');
        mediaTagged = true;
      } else {
        child.classList.add('accordion-item-content');
      }
    });
    ...
    // Normalize any authored image alt text inside the accordion content.
  ...
  const getBody = (detailsEl) => detailsEl.querySelector('.accordion-item-body');
  ...
  // Single delegated transitionend handler on the accordion block
  const onTransitionEnd = ({ target, propertyName }) => {
    if (!target || !target.classList || propertyName !== 'height') return;
    if (!target.classList.contains('accordion-item-body')) return;

    const detailsEl = target.closest('.accordion-item');
    if (!detailsEl) return;
  ...
  function updateFirstItemOpenClass() {
    if (block.classList.contains('accordion-stacked-right')) {
  ...
  // Helper to manage tabindex on interactive elements within accordion content
  const updateContentTabindex = (detailsEl, isExpanded) => {
    const body = getBody(detailsEl);
    if (!body) return;

    // Find all interactive elements within the accordion content
  ...
  const toggleItem = (detailsEl) => {
    ...
    // Get accordion item details for tracking
    const accordionIndex = items.indexOf(detailsEl);
    const summary = detailsEl.querySelector('summary');
    const accordionTitle = (summary?.textContent || '').trim();
    const action = detailsEl.open ? 'close' : 'open';

    // Track accordion toggle event with error handling
    try {
      trackElementInteraction('accordion-toggle', {
      ...
      console.warn('Failed to track accordion toggle event:', error);
    }
  ...
  // Track accordion initialization with error handling
  try {
    trackElementInteraction('accordion-init', {
      elementType: 'accordion',
      additionalData: {
        totalItems: items.length,
      },
    ...
    // eslint-disable-next-line no-console
    console.warn('Failed to track accordion initialization event:', error);
  }
}
