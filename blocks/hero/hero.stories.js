...
A flexible hero component that displays primary content at the top of a webpage with title, description, optional media (image/video), and CTA buttons.
...
### CSS Classes:
- \`.hero\` - Main block
- \`.hero-text-only\` - Text-only layout variant
- \`.hero-contains-media\` - Contains-media layout variant
- \`.hero-section\` - Section wrapper with ARIA attributes
- \`.hero-inner\` - Inner content container
- \`.hero-grid\` - Grid layout for contains-media
- \`.hero-content\` - Content wrapper
- \`.hero-pretitle\` - Pretitle text
- \`.hero-title\` - Main title heading
- \`.hero-description\` - Description text
- \`.hero-cta\` - CTA button wrapper
- \`.hero-media\` - Media wrapper
- \`.hero-caption\` - Media caption

### Universal Editor Support:
This block is fully compatible with Universal Editor, using the content model defined in \`_hero.json\`.
...
const createBlock = (args) => {
  ...
  block.className = HERO_CSS_CLASSES.BLOCK;
  ...
  if (args.mediaType === 'image' && args.layout === 'contains-media') {
    ...
    img.alt = args.altText || 'Hero image';
    img.loading = 'eager';
...
const Template = (args) => {
  ...
  section.className = HERO_CSS_CLASSES.CONTAINER;
  section.setAttribute('data-section-status', 'loaded');

  // Create block wrapper
  const wrapper = document.createElement('div');
  wrapper.className = HERO_CSS_CLASSES.WRAPPER;
...
TextOnlyMinimal.args = {
  layout: 'text-only',
  title: 'Simple Hero',
  description: 'A minimal hero with just title and description.',
  pretitle: '',
  ctaButtonText: '',
  ctaButtonLink: '',
};
