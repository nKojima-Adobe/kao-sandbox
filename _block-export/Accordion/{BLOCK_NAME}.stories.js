...
Accessible accordion with variants. Titles can include inline icons via ":icon-name:" syntax handled by the block.
  ...
  args: {
    '{BLOCK_VAR}Header': 'Frequently Asked Questions',
...
`,
  ...
  argTypes: {
    '{BLOCK_VAR}Header': { control: 'text', description: 'Optional heading rendered above the accordion' },
    styleVariation: {
      control: 'select',
...
// Icons are decorated by the block via decorateIcons in {BLOCK_NAME}.js
...
const Template = (args) => {
  ...
  if (args['{BLOCK_VAR}Header']) block.setAttribute('data-{BLOCK_NAME}-header', tokenizeIcons(args['{BLOCK_VAR}Header']));
...
export const {BLOCK_FUNC} = Template.bind({});
{BLOCK_FUNC}.parameters = {
  docs: {
