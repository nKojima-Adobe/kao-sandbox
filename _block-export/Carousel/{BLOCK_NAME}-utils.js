...
import {
  {BLOCK_UPPER}_PAUSE_VIDEO_ARIA_LABEL,
  {BLOCK_UPPER}_PLAY_VIDEO_ARIA_LABEL,
  {BLOCK_UPPER}_SLIDE_ANNOUNCEMENT,
 ...
 */
export function isValid{BLOCK_FUNC}Url(url) {
...
class VideoControlState {
  ...
  updateState(playing) {
    ...
    if (playing) {
      this.videoControlIconSpan.className = '{BLOCK_CLASS}-video-control-icon {BLOCK_CLASS}-video-control-pause';
      this.carouselVideoControlBtn.setAttribute('aria-label', this.placeholders[{BLOCK_UPPER}_PAUSE_VIDEO_ARIA_LABEL] || 'Pause video');
      this.carouselVideoControlBtn.setAttribute('aria-pressed', 'true');
    } else {
      this.videoControlIconSpan.className = '{BLOCK_CLASS}-video-control-icon {BLOCK_CLASS}-video-control-play';
      this.carouselVideoControlBtn.setAttribute('aria-label', this.placeholders[{BLOCK_UPPER}_PLAY_VIDEO_ARIA_LABEL] || 'Play video');
 ...
 */
export function create{BLOCK_FUNC}VideoControlButton(placeholders = {}) {
  const carouselVideoControlBtn = document.createElement('button');
  carouselVideoControlBtn.className = '{BLOCK_CLASS}-video-control';
  carouselVideoControlBtn.setAttribute('aria-label', placeholders[{BLOCK_UPPER}_PAUSE_VIDEO_ARIA_LABEL] || 'Pause video');
  ...
  videoControlIconSpan.className = '{BLOCK_CLASS}-video-control-icon {BLOCK_CLASS}-video-control-pause';
 ...
 */
export async function init{BLOCK_FUNC}VideoControls(mediaWrapper, videoJsEl, placeholders = {}) {
  const videoId = videoJsEl.getAttribute('id');
  if (!videoId) return null;

  // Create custom control button
  const carouselVideoControlBtn = create{BLOCK_FUNC}VideoControlButton(placeholders);
...
export function createLiveRegion(container) {
  ...
  liveRegion.className = '{BLOCK_CLASS}-live-region';
...
export function announceSlideChange(
  ...
  const template = placeholders[{BLOCK_UPPER}_SLIDE_ANNOUNCEMENT] || 'Slide {0} of {1}';
