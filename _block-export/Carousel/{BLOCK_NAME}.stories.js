...
- **Note**: Use real Brightcove video IDs in Storybook for videos to load and play
  ...
  argTypes: {
    layout: {
      ...
      description: 'Layout variation for the {BLOCK_NAME} block.',
...
const buildCarouselSlideRow = (slide) => {
  ...
  if (videoId && mediaType === 'video') {
    ...
    // Ensure video-js has an ID (required by {BLOCK_NAME} video loading logic)
    if (!videoJs.id) {
...
const Template = (args) => {
  ...
  // Decorate with {BLOCK_NAME} logic and icons
  decorate(block);
  decorateIcons(block);
...
FullGridImages.parameters = {
  docs: {
    description: {
      story: 'Standard full-grid layout with multiple image-based slides (more than five) to exercise full {BLOCK_NAME} navigation.',
    },
  },
};
...
FullWidthMixedMedia.parameters = {
  docs: {
    description: {
      story: 'Full-width layout combining Brightcove video and image slides. The video slide uses a real Brightcove video ID and will load and play automatically when the {BLOCK_NAME} initializes.',
    },
  },
};
...
SingleSlideMinimal.parameters = {
  docs: {
    description: {
      story: 'Minimal example with a single image slide for the {BLOCK_NAME}.',
    },
  },
};
...
ManySlides.parameters = {
  docs: {
    description: {
      story: '{BLOCK_FUNC} configured with eight slides to demonstrate behavior with more than five items.',
    },
  },
};
