 ...
 * Supports {BLOCK_CLASS}-item as child components
 */
...
import {
  {BLOCK_UPPER}_LINKED_PAGE_ARIA_LABEL,
  {BLOCK_UPPER}_GO_TO_ARIA_LABEL,
  {BLOCK_UPPER}_SLIDE_ARIA_LABEL,
  {BLOCK_UPPER}_SLIDE_ROLE,
  {BLOCK_UPPER}_PREVIOUS_SLIDE_ARIA_LABEL,
  {BLOCK_UPPER}_NEXT_SLIDE_ARIA_LABEL,
  {BLOCK_UPPER}_GO_TO_SLIDE_ARIA_LABEL,
  {BLOCK_UPPER}_ARIA_LABEL,
  {BLOCK_UPPER}_ROLE,
...
import {
  init{BLOCK_FUNC}VideoControls,
  isValid{BLOCK_FUNC}Url,
  decodeHtmlEntities,
  createElement,
  createLiveRegion,
  announceSlideChange,
 ...
 */
async function parse{BLOCK_FUNC}Config(block) {
...
function applyLayoutClasses(block, config) {
  ...
  if (config.layout) {
    layoutClass = `{BLOCK_CLASS}-layout-${config.layout}`;
    ...
    LAYOUT_VARIATIONS.forEach((variant) => {
      ...
      if (config[camelVariant] || config[variant]) {
        layoutClass = `{BLOCK_CLASS}-layout-${variant}`;
        block.classList.add(layoutClass);
        foundVariation = true;
      }
    });
    ...
    if (!foundVariation) {
      layoutClass = '{BLOCK_CLASS}-layout-full-grid';
      block.classList.add(layoutClass);
    }
  }
    ...
    if (codeEl) {
      ...
      if (media) {
        const bg = createElement('div', {
          className: '{BLOCK_CLASS}-media {BLOCK_CLASS}-media-bg',
          parent: item,
        });
...
function loadBrightcovePlayerForSlide(wrapper, media, placeholders) {
  try {
    ...
    if (media.tagName && media.tagName.toLowerCase() === 'video-js' && !media.getAttribute('id')) {
      const uniqueId = `{BLOCK_NAME}-video-${Date.now()}-${Math.floor(Math.random() * 100000)}`;
    ...
    if (isScriptAlreadyLoaded) {
      {BLOCK_VAR}InitQueue.add(() => {
        ...
        const initializeSlide = () => new Promise((resolve) => {
          ...
          init{BLOCK_FUNC}VideoControls(wrapper, media, placeholders)
...
function configureMedia(media, itemMeta, item, placeholders, shouldLoadMedia = false) {
  ...
  if (!item.querySelector('.{BLOCK_CLASS}-media')) {
    const mediaWrapper = createElement('div', {
      className: '{BLOCK_CLASS}-media',
      parent: item,
    });
  ...
  if (mediaTag === 'video') {
    const wrapper = media.closest('.{BLOCK_CLASS}-media');
    if (wrapper && !wrapper.classList.contains('{BLOCK_CLASS}-media-bg')) {
      wrapper.classList.add('{BLOCK_CLASS}-media-bg');
    }
    ...
    if (!media.getAttribute('id')) {
      const uniqueId = `{BLOCK_NAME}-video-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
      media.setAttribute('id', uniqueId);
    }
...
function loadSlideMedia(item, placeholders) {
  ...
  if (item.dataset.mediaLazyLoad === 'true') {
    ...
    if (media) {
      const wrapper = media.closest('.{BLOCK_CLASS}-media, .{BLOCK_CLASS}-media-bg');
...
function buildOverlay(temp, itemMeta) {
  const overlay = createElement('div', { className: '{BLOCK_CLASS}-overlay' });
  ...
  if (descriptionValue) {
    createElement('p', {
      className: '{BLOCK_CLASS}-description',
  ...
  if (ctaTextRaw && ctaLink && isValid{BLOCK_FUNC}Url(ctaLink)) {
    const cta = createElement('a', {
      className: 'button {BLOCK_CLASS}-cta',
...
function addClickableOverlay(item, slideLink, itemMeta, placeholders, options = {}) {
  if (!slideLink || !isValid{BLOCK_FUNC}Url(slideLink)) return;

  const titleForLabel = itemMeta.title
    ? (itemMeta.title || '').toString().replace(/<[^>]*>/g, '').trim()
    : (placeholders[{BLOCK_UPPER}_LINKED_PAGE_ARIA_LABEL] || 'linked page');
  const goToTemplate = placeholders[{BLOCK_UPPER}_GO_TO_ARIA_LABEL] || 'Go to {0}';

  const { allowTabFocus = false } = options;

  const clickableOverlay = createElement('a', {
    className: '{BLOCK_CLASS}-slide-link',
    attributes: {
      href: slideLink,
...
function getMediaLabelForSlide(item) {
  ...
  if (videoMedia) {
    ...
    const videoWrapper = videoMedia.closest('.{BLOCK_CLASS}-media, .{BLOCK_CLASS}-media-bg') || item;
  ...
  if (imageMedia) {
    ...
    const imageWrapper = imageMedia.closest('.{BLOCK_CLASS}-media, .{BLOCK_CLASS}-media-bg') || item;
...
function buildSlideAriaLabel(item, slideIndex, totalSlides, placeholders) {
  const slideTemplate = placeholders[{BLOCK_UPPER}_SLIDE_ARIA_LABEL] || 'Slide {0} of {1}';
...
function buildSlideFromRow(
  ...
  if (isImageOnlyLayout && rowHasVideoContent(row)) {
    ...
    if (document.documentElement.classList.contains('adobe-ue-edit')) {
      ...
      row.setAttribute('data-{BLOCK_NAME}-filtered', 'video-content');
    }
  ...
  item.className = '{BLOCK_CLASS}-item';

  // In authoring mode, clone content to preserve original row structure for UE editing
  // This ensures images remain as children of their {BLOCK_CLASS}-item rows in the UE
  ...
  item.setAttribute('aria-roledescription', placeholders[{BLOCK_UPPER}_SLIDE_ROLE] || 'slide');
 ...
 */
async function build{BLOCK_FUNC}Slides(rows, track, placeholders, layoutClass = null) {
 ...
 */
function create{BLOCK_FUNC}UI() {
  const container = createElement('div', { className: '{BLOCK_CLASS}-container' });
  const viewport = createElement('div', { className: '{BLOCK_CLASS}-viewport', parent: container });
  const track = createElement('div', { className: '{BLOCK_CLASS}-track', parent: viewport });
...
function createNavigationArrows(container, placeholders) {
  const prev = createElement('button', {
    className: '{BLOCK_CLASS}-nav {BLOCK_CLASS}-prev',
    innerHTML: '<span class="icon icon-arrow-backward"></span>',
    attributes: {
      'aria-label': placeholders[{BLOCK_UPPER}_PREVIOUS_SLIDE_ARIA_LABEL] || 'Previous slide',
    },
    parent: container,
  });

  const next = createElement('button', {
    className: '{BLOCK_CLASS}-nav {BLOCK_CLASS}-next',
    innerHTML: '<span class="icon icon-arrow-forward"></span>',
    attributes: {
      'aria-label': placeholders[{BLOCK_UPPER}_NEXT_SLIDE_ARIA_LABEL] || 'Next slide',
    },
    parent: container,
  });
...
function createPaginationDots(items, container, placeholders) {
  const dots = createElement('div', { className: '{BLOCK_CLASS}-dots' });

  const dotButtons = items.map((_, i) => {
    const goToSlideTemplate = placeholders[{BLOCK_UPPER}_GO_TO_SLIDE_ARIA_LABEL] || 'Go to slide {0}';
    return createElement('button', {
      className: '{BLOCK_CLASS}-dot',
      attributes: {
  ...
  if (isMobile && items[0]) {
    const firstMedia = items[0].querySelector('.{BLOCK_CLASS}-media, .{BLOCK_CLASS}-media-bg');
    if (firstMedia) {
...
function createNavigationController(items, container, navigationElements, placeholders) {
  ...
  const handleContainerPointerOver = (event) => {
    ...
    const targetSlide = eventTarget.closest('.{BLOCK_CLASS}-item');
  ...
  const updateUI = (userInitiated = false) => {
    ...
    if (isMobile && dots && dots.parentElement) {
      ...
      if (currentActiveSlide) {
        const currentActiveMedia = currentActiveSlide.querySelector('.{BLOCK_CLASS}-media, .{BLOCK_CLASS}-media-bg');
  ...
  const goTo = (index, placeholdersForLazyLoad, userInitiated = false, navigationAction = 'dot') => {
    ...
    if (container && typeof container.dataset.{BLOCK_VAR}Id !== 'undefined') {
      try {
        ...
        if (currentSlideElement) {
          const titleElement = currentSlideElement.querySelector('.{BLOCK_CLASS}-title');
          if (titleElement) {
        ...
        trackElementInteraction('{BLOCK_CLASS}-slide', {
          elementType: '{BLOCK_NAME}',
          elementId: container.dataset.{BLOCK_VAR}Id,
          additionalData: {
    ...
    resizeFrameId = requestAnimationFrame(() => {
      ...
      if (newIsMobile !== initialIsMobile) {
        if (newIsMobile) {
          ...
          if (activeSlide) {
            const activeMedia = activeSlide.querySelector('.{BLOCK_CLASS}-media, .{BLOCK_CLASS}-media-bg');
            if (activeMedia) {
...
function initScrollAnimations(block, items, resetAutoplay, getActiveIndex) {
  ...
  const startCarouselAnimations = (resetAutoplayFn) => {
    ...
    const hasFullGrid = block.classList.contains('{BLOCK_CLASS}-layout-full-grid');
    const isInsideGridColumns = block.closest('.grid-columns') !== null;
    if (hasFullGrid && !isInsideGridColumns) {
      block.classList.add('{BLOCK_CLASS}-expanded');
    }
        ...
        requestAnimationFrame(() => {
          if (currentActiveSlide) {
            ...
            if (items.length > 1 && resetAutoplayFn) {
              const currentCta = currentActiveSlide.querySelector('.{BLOCK_CLASS}-cta');
              if (currentCta) {
  ...
  const handleCarouselScroll = () => {
    ...
    scrollFrameId = requestAnimationFrame((timestamp) => {
      try {
        ...
        const isFullGrid = block.classList.contains('{BLOCK_CLASS}-layout-full-grid');
        const isExpanded = block.classList.contains('{BLOCK_CLASS}-expanded');
        ...
        if (isAtTop && isFullGrid && isExpanded && !isCollapsing) {
          if (timestamp - lastCollapseTime >= COLLAPSE_DEBOUNCE_MS) {
            isCollapsing = true;
            block.classList.remove('{BLOCK_CLASS}-expanded');
...
export default async function decorate(block) {
  ...
  if (block.dataset.{BLOCK_VAR}Decorated) {
    return;
  }
  block.dataset.{BLOCK_VAR}Decorated = 'true';

  // Generate unique ID for this carousel instance
  const {BLOCK_VAR}Id = generate{BLOCK_FUNC}UID();
  ...
  // Parse configuration
  const { config, rows } = await parse{BLOCK_FUNC}Config(block);

  // Reset any previously hidden rows (in case layout variation changed)
  rows.forEach((row) => {
    if (row.hasAttribute('data-{BLOCK_NAME}-filtered')) {
      row.style.display = '';
      row.removeAttribute('data-{BLOCK_NAME}-filtered');
    }
  });
  ...
  // Create UI structure
  const { container, viewport, track } = create{BLOCK_FUNC}UI();

  // Store carousel ID on container for tracking
  container.dataset.{BLOCK_VAR}Id = {BLOCK_VAR}Id;

  // Build slides (pass layoutClass for filtering)
  const items = await build{BLOCK_FUNC}Slides(rows, track, placeholders, layoutClass);
  ...
  if (isImageOnlyLayout) {
    ...
    const buttonsWrapper = createElement('div', { className: '{BLOCK_CLASS}-buttons' });
    ...
    container.setAttribute('aria-label', placeholders[{BLOCK_UPPER}_ARIA_LABEL] || 'Image Gallery');
    ...
    try {
      ...
      trackElementInteraction('{BLOCK_CLASS}-init', {
        elementType: '{BLOCK_NAME}',
        elementId: {BLOCK_VAR}Id,
        additionalData: {
          totalSlides,
          autoPlay,
        },
  ...
  items.forEach((item) => {
    const ctaButton = item.querySelector('.{BLOCK_CLASS}-cta');
    if (ctaButton && container.dataset.{BLOCK_VAR}Id) {
      const handleCtaClick = () => {
        try {
          ...
          trackElementInteraction('{BLOCK_CLASS}-cta-click', {
            elementType: '{BLOCK_NAME}',
            elementId: container.dataset.{BLOCK_VAR}Id,
            elementText: ctaText,
      ...
      if (!block.{BLOCK_VAR}CtaCleanup) {
        block.{BLOCK_VAR}CtaCleanup = [];
      }
      block.{BLOCK_VAR}CtaCleanup.push(() => {
  ...
  container.setAttribute('role', 'region');
  container.setAttribute('aria-label', placeholders[{BLOCK_UPPER}_ARIA_LABEL] || 'Carousel');
  container.setAttribute('aria-roledescription', placeholders[{BLOCK_UPPER}_ROLE] || 'carousel');

  // Create cleanup function
  block.{BLOCK_VAR}Cleanup = () => {
    // Cleanup CTA button click listeners
    if (block.{BLOCK_VAR}CtaCleanup && Array.isArray(block.{BLOCK_VAR}CtaCleanup)) {
      block.{BLOCK_VAR}CtaCleanup.forEach((cleanupFn) => {
        try {
          cleanupFn();
        } catch (error) {
          // eslint-disable-next-line no-console
    ...
    if (items && Array.isArray(items)) {
      items.forEach((item) => {
        if (item) {
          const mediaWrapper = item.querySelector('.{BLOCK_CLASS}-media-bg');
  ...
  const originalCleanup = block.{BLOCK_VAR}Cleanup;
  block.{BLOCK_VAR}Cleanup = () => {
    originalCleanup();
